...
def main():
    ...
    run_water_file(infile1, infile2, outfile)


def run_water_file(infile1, infile2, outfile):
    cline = WaterCommandline(asequence= infile1, bsequence = infile2, gapopen = 10, gapextend = 0.5, outfile = outfile)
...
def run_water(in_record1, in_record2):
    pipe_a, pipe_b = "/tmp/seq_pipe_a", "/tmp/seq_pipe_b"
    os.mkfifo(pipe_a), os.mkfifo(pipe_b)
    try:
        cline = WaterCommandline(asequence = pipe_a, bsequence = pipe_b, gapopen = 10, gapextend = 0.5, outfile="/dev/stdout")
        child = subprocess.Popen(str(cline), 
